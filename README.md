...
<p>
  ...
  <h3>⬇️ <a href="https://github.com/saunders77/TapConductor/releases/download/TapConductor/v0.1.0_TapConductor_universal.dmg">Download Mac installer v0.1.0_TapConductor_universal.dmg</a></h3>
  <h3>⬇️ <a href="https://github.com/saunders77/TapConductor/releases/download/TapConductor/v0.1.0_TapConductor_universal.dmg">Download Windows installer v0.1.0_TapConductor_x64-setup.exe</a></h3>
</p>
...
<h2>Configuring audio settings</h2>
    <p>Use the <b>AUDIO</b> control to select the speakers or sound card to use. On Windows, an option marked (ASIO) has an installed ASIO driver and may provide better latency on supported hardware. A driver such as ASIO4ALL can route to built-in Realtek speakers or headphones after that endpoint is enabled in the driver's control panel. ASIO is not automatically the best choice for every device or configuration; choose the output that is stable and responsive with your hardware.</p>
    <p id="instrument-help">Choose an instrument on the <b>Sound</b> menu, either the grand piano or a synthesizer.</p>
    <p>If you want to control TapConductor with a <strong>piano</strong> or another MIDI instrument, then plug in the instrument and select it from the MIDI In menu. You'll still be able to tap using normal mouse and keyboard controls too. When you use a piano, TapConductor will use the dynamics you play for each note, and you can use a sustain pedal. If you connect or reconnect a device while TapConductor is open, choose <b>Reload audio &amp; MIDI devices</b> from <b>AUDIO</b>.</p>
    <p>Piano shortcuts let you navigate without leaving the keyboard. Hold the shortcut function note (C2 by default and configurable in Info), then press E to go forward, D to go back, Dâ™¯ to replay the current chord, Câ™¯ to return to the beginning, or B to toggle direct MIDI play. The second note can be in any octave. Cmd/Ctrl+. also toggles direct MIDI play.</p>
