; Based on Tauri 2.11.4's default NSIS template. Keep this file in sync when
; upgrading Tauri; TapConductor's custom finish-page telemetry option is below.
Unicode true
...
Function FinishPageLeave
  SendMessage $TelemetryCheckbox ${BM_GETCHECK} 0 0 $0
  CreateDirectory "$APPDATA\${BUNDLEID}"
  ClearErrors
  FileOpen $1 "$APPDATA\${BUNDLEID}\telemetry-install-consent-v1" w
  ${IfNot} ${Errors}
    ${If} $0 = ${BST_CHECKED}
      FileWrite $1 "enabled"
    ${Else}
      FileWrite $1 "disabled"
    ${EndIf}
    FileClose $1
  ${EndIf}
FunctionEnd
