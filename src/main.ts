    ...
    <div id="help-overlay" class="help-overlay hidden" role="dialog" aria-modal="true" aria-labelledby="help-title">
      <section class="help-card">
        ...
        <div class="help-content">
          ...
          <section id="help-instructions" tabindex="-1"><h3>Open a sheet music score</h3><p>Select a MusicXML, compressed MusicXML, or MIDI file (file extensions <b>.musicxml</b>, <b>.xml</b>, <b>.mxl</b>, <b>.mid</b>, or <b>.midi</b>). If you use notation software (like MuseScore, Sibelius, or Dorico) or a DAW (like Ableton Live, Logic Pro, or Cubase), you can use the Export function to create a MusicXML or MIDI file that TapConductor can read. If you only have a PDF, you can use a converter program to create a file TapConductor can read (free options like Audiveris or MuseScore or paid options like SmartScore, PlayScore, Soundslice, NewZik, or PhotoScore).</p><p class="help-demo-links">Or open one of the included examples: <a id="help-demo-choir-open" href="#">demo choir score</a> or <a id="help-demo-piano-open" href="#">demo piano score</a>.</p></section>
          ...
          <section><h3>Conduct the score</h3>
            ...
            <p>By default, all staves (parts) will play during tapping, but you can select specific staves in the <b>PARTS</b> menu.</p>
          ...
          <section id="privacy" class="legal-disclosure" tabindex="-1">
            ...
            <p>TapConductor does not request access to your microphone, camera, location, contacts, or photos. The full policy is available in PRIVACY.md and at <span class="legal-url">github.com/saunders77/TapConductor</span>.</p>
          </section>
          <section id="acknowledgements" class="legal-disclosure" tabindex="-1">
            ...
            <p>TapConductor also uses open-source Tauri, OpenSheetMusicDisplay, Rust, TypeScript, and supporting libraries. TapConductor for Windows is distributed under the GNU General Public License version 3. For all other platforms it's distributed under the MIT license. Complete dependency and instrument notices are in the bundled THIRD_PARTY_NOTICES.md.</p>
          </section>
      ...
      <section class="score-panel" aria-label="Musical score">
        ...
        <div id="score-scroll" class="score-scroll" role="region" tabindex="0" aria-label="Score viewer" aria-describedby="score-keyboard-help">
          <div id="empty-state" class="empty-state">
            ...
            <p>If you use notation software<span class="empty-landscape-extra"> (like MuseScore, Sibelius, or Dorico)</span> or a DAW<span class="empty-landscape-extra"> (like Ableton Live, Logic Pro, or Cubase)</span>, you can use the Export function to create a MusicXML or MIDI file that TapConductor can read. If you only have a PDF, you can use a converter program to create a file TapConductor can read<span class="empty-landscape-extra"> (free options like Audiveris or MuseScore or paid options like SmartScore, PlayScore, Soundslice, NewZik, or PhotoScore)</span>.</p>
...
const COMPATIBILITY_MOUSE_BLOCK_MS = 1_500;
let compatibilityMouseBlockedUntil = 0;
type PointerHold = {
  token: string;
...
function noteNonMousePointerEvent(event: PointerEvent): void {
  if (event.pointerType !== "mouse") {
    compatibilityMouseBlockedUntil = performance.now() + COMPATIBILITY_MOUSE_BLOCK_MS;
  }
}
...
function isTouchCompatibilityMouseEvent(event: MouseEvent): boolean {
  ...
  return sourceCapabilities?.firesTouchEvents === true
    || performance.now() < compatibilityMouseBlockedUntil;
}
...
app.addEventListener("mousedown", (event) => {
  if (event.button !== 0 || isTouchCompatibilityMouseEvent(event)) return;
  const match = auditionTargetFromEvent(event);
...
elements.tap.addEventListener("mousedown", (event) => {
  if (
    event.button !== 0
    || isTouchCompatibilityMouseEvent(event)
    || isPointerManagedButtonPressed(elements.tap)
  ) return;
  event.preventDefault();
...
function consumeCompletedButtonClick(
  ...
  // HTMLElement.click() is untrusted and represents an intentional recovery or
  // keyboard/programmatic activation. WebKit's delayed touch click is trusted,
  // but may still report detail 0, so detail cannot distinguish it reliably.
